...
void MPP_MultiPartInit(MultipartProcessor* theProcessor, char* theBoundStr, int theBoundStrLen)
{
    ...
    theProcessor->cb_HeaderName = NULL;
    theProcessor->cb_HeaderValue = NULL;
    theProcessor->cb_HeaderParameterName = NULL;
    theProcessor->cb_HeaderParameterValue = NULL;
  ...
  while( theProcessor->m_Indx < theBufferSize ){
    switch(theProcessor->m_State){
      ...
      case MPS_HEADER_NAME:{
        ...
        if( theBuffer[theProcessor->m_Indx] == ':'){
          if( theProcessor->cb_HeaderName ){
            aSize = theProcessor->m_Indx-aSectionStart;
            mpp_trim(theBuffer,&aSectionStart,&aSize);
            aRes = theProcessor->cb_HeaderName(theBuffer+aSectionStart, aSize, theProcessor->m_CBContext);
            if( aRes != 0 ){
              return aRes;
            }
      ...
      case MPS_HEADER_VALUE:{
        if( theBuffer[theProcessor->m_Indx] == ';' || theBuffer[theProcessor->m_Indx] == '\r' || theBuffer[theProcessor->m_Indx] == '\n'){
          if( theProcessor->cb_HeaderValue ){
            aSize = theProcessor->m_Indx-aSectionStart;
            mpp_trim(theBuffer,&aSectionStart,&aSize);
            aRes = theProcessor->cb_HeaderValue(theBuffer+aSectionStart, aSize, theProcessor->m_CBContext);
            if( aRes != 0 )
              return aRes;
          }
          if( theBuffer[theProcessor->m_Indx] == '\r' || theBuffer[theProcessor->m_Indx] == '\n'){
...
/*Parse parameter name*/
        ...
        if( theBuffer[theProcessor->m_Indx] == '=' ){
          ...
          if( theProcessor->cb_HeaderParameterName ){
            aSize = theProcessor->m_Indx-aSectionStart;
            mpp_trim(theBuffer,&aSectionStart,&aSize);
            aRes = theProcessor->cb_HeaderParameterName(theBuffer+aSectionStart, aSize, theProcessor->m_CBContext);
            if( aRes != 0 )
              return aRes;
          }
          aSectionStart = theProcessor->m_Indx + 1;
...
/*Parse parameter value*/
        if( theBuffer[theProcessor->m_Indx] == ';' || theBuffer[theProcessor->m_Indx]=='\r' || theBuffer[theProcessor->m_Indx]=='\r' ){
          if( theProcessor->cb_HeaderParameterValue ){
            aSize = theProcessor->m_Indx-aSectionStart;
            mpp_trim(theBuffer,&aSectionStart,&aSize);
            mpp_remove_quotas(theBuffer,&aSectionStart,&aSize);
            aRes = theProcessor->cb_HeaderParameterValue(theBuffer+aSectionStart, aSize, theProcessor->m_CBContext);
            if( aRes != 0 )
              return aRes;
          }
          if( theBuffer[theProcessor->m_Indx] == ';'){
